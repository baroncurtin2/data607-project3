...
sf_separateCols <- cbind(sf_separateCols, termsDf) 
...
for(i in 3:ncol(sf_nonNA)) {
  nonNA[, i] <- str_count(sf_nonNA$requirements, str_c(c("([^[:alpha:]]", "[[:punct:][:blank:]]?", colnames(sf_nonNA)[i], "[[:punct:][:blank:]]{1})"), collapse = ""))
}
...
write_csv(sf_jobPostings, "./data/jobpostings.csv")
write_csv(sf_keywordSummary, "./data/keywordsummary.csv")
```
